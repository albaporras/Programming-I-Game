...
public class GuiJoc implements KeyListener {
    ...
    private void actualitzarJoc() {
       ...
       if (arqueologa.isMort()){
            logica.setEstat(LogicaJoc.EstatJoc.GAMEOVER);
            arqueologa.setDiamantes(vacia);
            arqueologa.setArcaEncontrada(false);
            arqueologa.setDiamantesEncontrados(false);
            int s = arqueologa.getDiamantes().length;
            for (int k = 0; k < s; k++){
                arqueologa.setComprobar(k, false);
            }
            
        }
        
        else if((arqueologa.getArcaEncontrada() == true) && (arqueologa.getDiamantesEncontrados()== true)){
            logica.setEstat(LogicaJoc.EstatJoc.GAMEOVER);
            arqueologa.setDiamantes(vacia);
            arqueologa.setArcaEncontrada(false);
            arqueologa.setDiamantesEncontrados(false);
    ...
    private void mostrarGameOver(BufferStrategy strategy) {
        ...
        // ... feu el codi aqui
        if(joc.getArqueologa().getVida()<=0.0f){
            ...
            g2.drawString(" ho has aconseguit! ", 160, 120);
            f = new Font("Dialog", Font.PLAIN, 16);
            g2.setFont(f);
